...
def processImage(path,outputdir):
    ...
    try:
        while True:
            ...
            new_frame.save(os.path.join(outputdir, '%s-%d.png' % (path, i)), 'PNG')
...
#    filename = raw_input('input the path of gif: ')
#    outputdir = raw_input('input the path of output dir: ')
    ...
    if filename[-4:] == '.gif':
        ...
        processImage(filename[1],outputdir)
        print 'done.'
    else:
